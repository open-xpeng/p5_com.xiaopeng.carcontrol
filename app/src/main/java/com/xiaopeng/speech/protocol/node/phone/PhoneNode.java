...
public class PhoneNode extends SpeechNode<PhoneListener> {
        ...
        if (!CarTypeUtils.isOverseasCarType() || CarTypeUtils.isE38EU()) {
            ...
            try {
                for (Contact.PhoneInfo phoneInfo : list) {
                    ...
                    if (!CarTypeUtils.isE38EU() && !TextUtils.isEmpty(name)) {
                        String replaceAll = name.replaceAll("[0-9]", "");
                        name = isEnglish(replaceAll) ? replaceAll.toUpperCase() : replaceAll.replaceAll("[a-zA-Z]", "");
                    }
    ...
    public void incomingCallRing(String str, String str2, boolean z) {
        ...
        String str3 = "";
        String str4 = !TextUtils.isEmpty(str2) ? str2 + "来电话了，要接听还是拒绝？" : "";
        if (!TextUtils.isEmpty(str)) {
            str4 = str + "来电话了，要接听还是拒绝？";
        }
        if (z2) {
            str4 = "来电话了，麦克风已禁用，通话中可在电话应用内取消静音哦";
        }
        try {
            str3 = new JSONObject().put("tts", str4).put("来电号码", str2).toString();
        } catch (JSONException e) {
            e.printStackTrace();
        }
        try {
            str3 = new JSONObject().put("isLocalSkill", true).put("来电人", str).put("来电号码", str2).put("tts", str4).put(WakeupResult.REASON_COMMAND, "command://phone.in.accept#command://phone.in.reject").put("enable_tts", z).toString();
        } catch (JSONException e2) {
            e2.printStackTrace();
        }
        SpeechClient.instance().getSpeechState().setCanExitFlag(false);
        SpeechClient.instance().getAgent().triggerIntentWithBinder(this.mBinder, "离线来电话", "来电话", "来电话", str3);
