...
public class SeatAirStatefulButtonConstructor extends BaseStatefulButtonConstructor {
    private int level;
    private String[] vuiLabels;

    public SeatAirStatefulButtonConstructor(Context mContext, int level) {
        this.level = level;
        if (mContext != null) {
    ...
    public void construct(IVuiElement vuiFriendly) {
        ...
        setStatefulButtonData(vuiFriendly, i2, 1, 3, 1.0f, this.vuiLabels);
